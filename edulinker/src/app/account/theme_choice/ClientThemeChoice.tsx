...
interface Template { id: string; nome: string; pro: boolean }
...
export default function ClientThemeChoice({ siteId }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const stRes = await fetch('/api/onboarding/status', { credentials: 'include' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/onboarding/template', {
        ...
        body: JSON.stringify({ siteId: id, templateId: selected, configuracoes: {} }),
      })
      if (!res.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-5xl mx-auto flex flex-col items-center px-4 space-y-8 pb-12">
      <h1 className="text-3xl font-bold text-gray-800 text-center">ESCOLHA O SEU TEMA</h1>
